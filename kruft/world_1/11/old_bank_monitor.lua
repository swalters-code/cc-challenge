...
-- Version: 1.8.0 (Added: Average Resource Usage)
...
local CONFIG = {
    ...
    TICK_RATE = 5,          -- seconds between inventory scans
...
for i, res in ipairs(CONFIG.RESOURCES) do
    state.resources[i] = {
        ...
        rate_str = "***",
        avg_str = "***",
        history = {}, -- Buffer for last 10 items/sec rates
        status = "OK"
    }
end
...
-- Accepts secondsPerItem
local function formatTime(seconds)
    if seconds == 0 then return "***" end
    if seconds < 1 then
        return string.format(">%.1f/s", 1/seconds)
    end
    return string.format("%ds/i", math.floor(seconds)) -- Shortened to fit layout
end
...
local function scanInventory()
    ...
    for i, config in ipairs(CONFIG.RESOURCES) do
        ...
        if totalCount < previous then
            ...
            -- Update History (Avg Calculation)
            table.insert(resState.history, 1, itemsPerSecond)
            if #resState.history > 10 then
                table.remove(resState.history)
            end
            
            -- Calculate Average Rate
            local sum = 0
            for _, rate in ipairs(resState.history) do
                sum = sum + rate
            end
            local avgRate = sum / #resState.history
            
            -- Convert back to seconds/item for display consistency
            if avgRate > 0 then
        ...
        elseif totalCount > previous then
            -- Refill detected
            resState.last_drop_time = currentTime
            -- Clear history on refill? Usually safer to keep or clear. 
            -- We'll keep to show trend before refill.
            
        else 
            -- Idle (totalCount == previous)
            if (currentTime - resState.last_drop_time) > 300 then
                resState.rate_str = "***"
                -- Avg fades to old? No, keep last known average until new data arrives or clear manually
            end
        end
...
--------------------------------------------------------------------------------
-- 5. Core Logic: Liveness & Aggregation
--------------------------------------------------------------------------------
...
local function calculateGridStats()
    ...
    local agg = {
        ...
        stored = 0,
        max = 0,
        gen_net = 0, -- Sum of reactor telemetry
        free = 0,
        cap_percent = 0
    }
    ...
    -- Calculate True Net (Rate of Change)
    local true_net = 0
    
    if state.grid.last_calc_time > 0 then
        local dt = now - state.grid.last_calc_time
        local ticks = dt * 20 -- Minecraft runs at 20 ticks per second
...
local function render()
    ...
    -- === ZONE A: Global Resources ===
    -- Adjusted layout to fit AVG column
    -- RES (9) | CNT (6) | % (3) | CUR (8) | AVG (8) | STS (3)
    ...
    for i, res in ipairs(CONFIG.RESOURCES) do
        ...
        -- Format string adapted for new layout
        local lineBase = string.format("%-9s | %-6s | %3d%% | %-8s | %-8s | ", 
            string.sub(res.label, 1, 9), 
            formatNumber(s.count), 
            math.floor(s.percent), 
            s.rate_str,
            s.avg_str
        )
    ...
    -- Draw Gen Net in parenthesis
    local genVal = formatNumber(g.gen_net)
    if g.gen_net >= 0 then genVal = "+" .. genVal end 
    
    local genStr = string.format(" (%s Gen)", genVal) 
    drawText(mon, 7 + #trueNetStr, yOffset+2, genStr, colors.gray)
    
    -- Stored Line
    local storedStr = string.format("Stored: %s / %s (%s) %d%%", 
        formatNumber(g.stored), 
        formatNumber(g.max), 
        formatNumber(g.free), 
        math.floor(g.cap_percent)
    )
    ...
    for idx, key in ipairs(sortedKeys) do
        ...
        local sColor = C_BAD
        if r.status == "online" then 
            sChar = "R"; sColor = C_GOOD 
        elseif r.status == "paused" then 
            sChar = "P"; sColor = C_WARN 
        end
        ...
        local metrics = string.format("  | %3d%% | %-8s | %-8s | %-4s | %s",
            pwr, netStr, avgStr, tempStr, resP
        )
        drawText(mon, 15, y, metrics, C_TEXT)
    end
end
...
local function telemetryListener()
    ...
    if modemSide then
        ...
        print("Listening on " .. modemSide .. " for: " .. CONFIG.PROTOCOL)
    else
        print("Warning: No wireless modem found for Rednet!")
    end
...
local function systemTick()
    ...
    while true do
        scanInventory()
        checkLiveness()
        calculateGridStats() -- Updates global stats before render
        render()
