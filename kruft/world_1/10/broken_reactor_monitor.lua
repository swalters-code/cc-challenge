...
-- CC:Tweaked Lua program for monitoring and controlling a Powah mod Uraninite Reactor
...
-- Telemetry
local REACTOR_ID = "reactor_01"  -- Change this for each reactor
...
local function getReactorData()
    ...
    elseif data.isRunning then
        data.status = "ONLINE"
    elseif data.storedEnergy >= data.maxEnergy then
        data.status = "PAUSED"
    else
        -- Should rarely happen if enabled but not running or paused
        data.status = "OFFLINE"
    end
...
local function getAlerts(data)
    ...
    if data.status == "OFFLINE" and isSystemEnabled then
        table.insert(alerts, "stopped")
    end
...
local function updateRedstoneControl()
    -- Control logic is now purely based on the manual switch
    redstone.setOutput(REDSTONE_OUTPUT_SIDE, isSystemEnabled)
end
...
local function updateDisplay(data, avgFET)
    ...
    local label = os.getComputerLabel() or "Reactor Monitor"
    writeAt(monitor, 1, line, centerText(label, width), colors.white)
    line = line + 1
    ...
    -- Line 3: Status Value (centered, color coded)
...
local function main()
    ...
    print("Touch monitor to toggle ON/OFF")
    ...
    while true do
        ...
        elseif event == "monitor_touch" then
            ...
            updateRedstoneControl()
            performUpdate() -- Immediate UI refresh
        end
    end
end
